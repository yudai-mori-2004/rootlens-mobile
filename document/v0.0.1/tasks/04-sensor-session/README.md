...
- [ ] `app/modules/sensor-session/` (iOS / Android) を root-lens v0.1.1 から移植
- [ ] `app/src/sensors/` (TS abstraction) を root-lens から移植
- [ ] `app/src/native/sensorSession.ts` (Expo Modules ブリッジ) を移植
- [ ] sidecar JSON schema (本 README §出力形式) で出力を整形
- [ ] `npx tsc --noEmit` pass
- [ ] `npx expo-doctor` pass
- [ ] Android (Pixel 10 もしくは Solana Seeker) で `clip_*.mp4` + `clip_*.json` が出る
- [ ] iOS で同上 (Mac で simulator または実機)
- [ ] sidecar の IMU sample 数 / frame timestamp が capture window と整合
